...
import {
  ShoppingCart, Search, Eye, Clock, CheckCircle, Truck, Package, XCircle, RefreshCw,
  Download, LayoutGrid, Printer, ClipboardCheck, AlertCircle, ArrowRight, X
} from 'lucide-react';

const ORDER_STATUSES = [
  { id: 'pending', label: 'Pending', color: 'yellow', icon: Clock, description: 'Awaiting confirmation' },
  { id: 'confirmed', label: 'Confirmed', color: 'blue', icon: CheckCircle, description: 'Payment received' },
  { id: 'processing', label: 'Processing', color: 'blue', icon: RefreshCw, description: 'Preparing artwork' },
  { id: 'printing', label: 'Printing', color: 'purple', icon: Printer, description: 'In production' },
  { id: 'quality_check', label: 'Quality Check', color: 'indigo', icon: ClipboardCheck, description: 'Final inspection' },
  { id: 'shipped', label: 'Shipped', color: 'emerald', icon: Truck, description: 'On the way' },
  { id: 'delivered', label: 'Delivered', color: 'emerald', icon: Package, description: 'Completed' },
  { id: 'cancelled', label: 'Cancelled', color: 'gray', icon: XCircle, description: 'Order cancelled' },
];
...
function OrderCard({ order, selected, onSelect, onQuickAction }) {
  ...
  return (
    <div className={`group relative rounded-lg border bg-white p-4 transition-all hover:shadow-md ${selected ? 'ring-2 ring-emerald-500' : ''}`}>
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-start gap-3">
          ...
          />
          <div>
            <Link href={`/admin/orders/${order.id}`} className="font-semibold text-gray-900 hover:text-emerald-600">
      ...
      <div className="mt-3 flex gap-2 opacity-0 transition-opacity group-hover:opacity-100">
        <Link href={`/admin/orders/${order.id}`} className="flex-1 rounded-lg border px-3 py-1.5 text-center text-xs font-medium text-gray-700 hover:bg-gray-50">
          View Details
        </Link>
        <button
...
export function OrdersPageClient({ initialOrders }) {
  ...
  const getNextStatus = (currentStatus) => {
    const flow = ['pending', 'confirmed', 'processing', 'printing', 'quality_check', 'shipped', 'delivered'];
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <Link href="/admin/orders/board" className="inline-flex flex-1 items-center justify-center gap-2 rounded-lg border bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 sm:flex-none">
      ...
      <div className="mb-6 overflow-x-auto">
        <div className="grid grid-cols-3 gap-3 sm:grid-cols-6 sm:gap-4" style={{ minWidth: '500px' }}>
          ...
          <Card className={`cursor-pointer hover:shadow-md ${stats.needsAction > 0 ? 'border-yellow-300 bg-yellow-50' : ''}`} onClick={() => setStatusFilter('pending')}>
            <CardContent className="p-3 sm:p-4">
          ...
          <Card className="cursor-pointer hover:shadow-md" onClick={() => setStatusFilter('shipped')}>
            <CardContent className="p-3 sm:p-4">
          ...
          <Card>
            <CardContent className="p-3 sm:p-4">
              ...
              <p className="text-lg font-bold text-emerald-600 sm:text-2xl">{formatCurrency(stats.revenue)}</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      {selectedOrders.length > 0 && (
        <div className="mb-4 flex flex-col gap-3 rounded-lg bg-emerald-50 p-3 sm:flex-row sm:items-center sm:gap-4 sm:p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button size="sm" variant="outline" onClick={() => handleBulkStatusUpdate('confirmed')} disabled={isUpdating}>
              <CheckCircle className="mr-1 h-4 w-4" /><span className="hidden sm:inline">Confirm</span>
            </Button>
            <Button size="sm" variant="outline" onClick={() => handleBulkStatusUpdate('processing')} disabled={isUpdating}>
              <RefreshCw className="mr-1 h-4 w-4" /><span className="hidden sm:inline">Process</span>
            </Button>
            <Button size="sm" variant="outline" onClick={() => handleBulkStatusUpdate('shipped')} disabled={isUpdating}>
              <Truck className="mr-1 h-4 w-4" /><span className="hidden sm:inline">Ship</span>
            </Button>
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y bg-white">
                    {filteredOrders.map(order => {
                      ...
                      const nextStatus = getNextStatus(order.status);
                      return (
                        <tr key={order.id} className={`hover:bg-gray-50 ${selectedOrders.includes(order.id) ? 'bg-emerald-50' : ''}`}>
                          ...
                          <td className="px-4 py-3 text-sm text-gray-500">{itemCount} item{itemCount !== 1 ? 's' : ''}</td>
